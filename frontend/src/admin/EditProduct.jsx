...
import { updateProduct } from "../services/productAdminService";

// Props: product (object already mapped by getAdminProducts — fields: id, name, price, description, gambar, status, category, category_id)
function EditProduct({ product, closeForm, refreshProducts }) {

    const [categories, setCategories] = useState([]);
    const [loading, setLoading] = useState(false);
    const [loadingCategories, setLoadingCategories] = useState(true);

    // ✅ FIX: Initialize form using the already-mapped field names from getAdminProducts()
    //   product.name → nama_produk (mapped by getAdminProducts)
    //   product.description → deskripsi (mapped by getAdminProducts)
    //   product.price → harga (mapped by getAdminProducts)
    //   product.category_id → kategori_id (mapped by getAdminProducts)
    //   product.gambar → unchanged
    //   product.status → unchanged
    // Removed 'slug' (not in backend) and 'stock' (not in backend UPDATE)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoading(true);
            // updateProduct() in productAdminService maps: name→nama_produk, price→harga, description→deskripsi, etc.
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-indigo-100 mb-6">
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">ID: {product.id} — {product.name}</p>
                </div>
                <button
                    type="button"
                    onClick={closeForm}
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                    ...
                    {/* ✅ FIX: name="name" matches form.name (mapped from nama_produk) */}
                    <input
                        type="text"
                        name="name"
                        value={form.name}
                ...
                <div>
                    ...
                    {/* ✅ FIX: name="price" matches form.price (mapped from harga) */}
                    <input
                        type="number"
                        name="price"
                        value={form.price}
                ...
                <div>
                    ...
                    {loadingCategories ? (
                        ...
                        <select
                            ...
                            onChange={handleChange}
                            className={`w-full border rounded-lg p-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white transition-all ${
                ...
                <div className="md:col-span-2">
                    ...
                    {/* ✅ FIX: name="description" matches form.description (mapped from deskripsi) */}
                    <textarea
                        name="description"
